...
fb.Router = Backbone.Router.extend({
    ...
    person: function (id) {
        var self = this;
        $('#content').html('<div class="breadcrumb api hide">FB.api("/' + id + '");</div>');
        try {
    ...
    //                 self.showErrorPage();
    //             } else {
    //                 console.log(new fb.views.Person({model: new fb.models.Person(response)}).el);
    //             }
    //         });
    //     } catch (e) {
    //         this.showErrorPage();
    ...
    friends: function (id) {
        var self = this;
        $('#friendlist').html('<div class="breadcrumb api hide">FB.api("/' + id + '/friends");</div>');
        try {
            FB.api("/" + id + "/friends?limit=5", function (response) {
                // console.log(response);
                if (response.error) {
                    self.showErrorPage();
                } else {
                    $('#friendlist').append(new fb.views.Friends({model: new Backbone.Model(response)}).el);
                }
            });
        } catch (e) {
            this.showErrorPage();
    ...
    mutualfriends: function (id) {
        var self = this;
        $('#content').html('<div class="breadcrumb api hide">FB.api("/' + id + '/mutualfriends");</div>');
        try {
...
$(document).on('click','.logout', function () {
    FB.logout();
    window.location.replace('http://localhost/inoxapps/');
    return false;
});
